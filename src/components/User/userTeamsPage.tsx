...
import { TeamIcon, TEAM_ICON_NAMES, TEAM_COLORS } from './CreateTeamModal';
import { useHookGetUser } from '../../hooks/useHookGetUser';
import { useHookDeleteTeam } from '../../hooks/useHookDeleteTeam';
import { useHookPatchTeamMeta } from '../../hooks/useHookPatchTeamMeta';
...
export function UserTeamsPage({ username }: { username: string }) {
  ...
  const { data, loading, error, refetch } = useHookGetUser(username);
  ...
  const saveEditMeta = async () => {
    ...
    try {
      ...
      setEditingTeam(null);
      refetch();
      window.dispatchEvent(new CustomEvent('coop_teams_updated'));
    } catch {}
  };
