...
import { BarChart2, Star, CheckSquare, TrendingUp, Trophy, Users, ChevronLeft, ChevronRight, Crown } from 'lucide-react';
...
/* ── Team Analytics Sub-Component (manager only) ── */
function TeamAnalyticsTab({ managerTeams }: { managerTeams: { id: number; name: string; role: string }[] }) {
  const [selectedTeamId, setSelectedTeamId] = useState(managerTeams[0]?.id ?? 0);
  const [profileMemberId, setProfileMemberId] = useState<number | null>(null);
  const { data: teamTasks, loading: tasksLoading, refresh: refreshTasks } = useHookGetTask(selectedTeamId);
                ...
                {memberStats.map((m, i) => (
                  <div key={m.memberId} className="an-member-row" style={{ cursor: 'pointer' }} onClick={() => setProfileMemberId(m.memberId)}>
                    <div className="an-member-info">
                      <div className="an-member-avatar" style={{ background: memberColor(m.memberId) }}>
            ...
            {leaderOfWeek && leaderOfWeek.weekPoints > 0 ? (
              <div className="card an-leader-card an-week-leader-card" style={{ background: `linear-gradient(135deg, ${memberColor(leaderOfWeek.memberId)}22, var(--bg-elev))` }}>
                ...
                <div className="an-leader-body">
                  <div className="an-leader-avatar" style={{ background: memberColor(leaderOfWeek.memberId) }}>
                  ...
                  {[...memberStats]
                    ...
                    .map((m, i) => (
                      <div key={m.memberId} className="an-week-row">
                        ...
                        <div className="an-member-avatar an-member-avatar--sm" style={{ background: memberColor(m.memberId) }}>
                          {(m.name || m.username).charAt(0).toUpperCase()}
                        </div>
...
export function PersonalStatisticsPage({ username }: PersonalStatisticsPageProps) {
  ...
  const [kpiTeam, setKpiTeam] = useState<number | null>(null);
  ...
  const managerTeams = user?.teams.filter(t => t.role === 'manager') ?? [];

  /* ── KPI strip ── */
  const kpis = useMemo(() => {
    const filteredAll      = kpiTeam != null ? allTasks.filter(t => t.teamId === kpiTeam)      : allTasks;
    const filteredAssigned = kpiTeam != null ? assignedTasks.filter(t => t.teamId === kpiTeam) : assignedTasks;
    const total    = filteredAll.length;
    const done     = filteredAll.filter(t => t.status === 'completed').length;
    const myPts    = filteredAssigned.filter(t => t.status === 'completed').reduce((s, t) => s + (t.points ?? 0), 0);
    ...
    return [
      ...
      { k: 'Мои очки',      v: myPts,      sub: `${filteredAssigned.length} назначенных`,       accent: 'var(--accent)',iconBg: 'rgba(91,141,239,.13)', Icon: Trophy },
  ...
  }, [allTasks, assignedTasks, user, kpiTeam]);
  ...
  const ratingData = useMemo(() => {
    ...
    return user.teams.map(team => {
      ...
      return { id: team.id, name: team.name, role: team.role, taken, closed, total, teamPct, color: memberColor(team.id) };
  ...
  const tabs: { key: AnalyticsMode; label: string }[] = [
    ...
    ...(isManager ? [{ key: 'team' as AnalyticsMode, label: 'Команда' }] : []),
  ];
  ...
  return (
    <div className="personal-statistics-container">
      ...
      {/* KPI team selector */}
      {(user?.teams.length ?? 0) > 1 && (
        <div className="an-team-selector">
          <button
            className={`an-team-sel-btn ${kpiTeam === null ? 'active' : ''}`}
            onClick={() => setKpiTeam(null)}
          >
            Все команды
          </button>
          {user!.teams.map(t => (
            <button
              key={t.id}
              className={`an-team-sel-btn ${kpiTeam === t.id ? 'active' : ''}`}
              onClick={() => setKpiTeam(kpiTeam === t.id ? null : t.id)}
            >
              {t.name}
            </button>
          ))}
          ...
          <div className="an-leader-col">
            {topTeam && (
              <div className="card an-leader-card" style={{ background: `linear-gradient(135deg, ${topTeam.color}22, var(--bg-elev))` }}>
                <div className="an-leader-eyebrow">лидер недели</div>
                <div className="an-leader-body">
                  <div className="an-leader-avatar" style={{ background: topTeam.color }}>
                      ...
                      {week.map((cell, di) => (
                        <div
                          ...
                          style={cell && cell.count > 0 ? {
                            background: `rgba(91,141,239,${Math.min(0.85, 0.18 + (cell.count / heatMax) * 0.67)})`,
                            borderColor: 'rgba(91,141,239,.45)',
                          } : {}}
                        >
                          {cell && (
                            <span
                              className="an-heatmap-day-num"
                              style={{ color: cell.count > 0 && (cell.count / heatMax) > 0.55 ? '#fff' : undefined }}
                            >
                              {cell.day}
                            </span>
                          )}
              ...
              <div className="an-filter-section" style={{ marginTop: 'auto' }}>
                ...
                <div className="an-heatmap-stats">
                  <div className="an-heatmap-stat">
                    <span className="mono" style={{ fontSize: 20, fontWeight: 800, color: 'var(--ink)' }}>
                      {heatmapMonthData.filter(d => d.count > 0).length}
                    </span>
                    <span style={{ fontSize: 11, color: 'var(--ink-3)' }}>активных дней</span>
                  </div>
                  <div className="an-heatmap-stat">
                    <span className="mono" style={{ fontSize: 20, fontWeight: 800, color: 'var(--accent)' }}>
                      {Math.max(0, ...heatmapMonthData.map(d => d.count))}
                    </span>
                    <span style={{ fontSize: 11, color: 'var(--ink-3)' }}>макс. за день</span>
                  </div>
                  <div className="an-heatmap-stat">
                    <span className="mono" style={{ fontSize: 20, fontWeight: 800, color: 'var(--good)' }}>
                      {heatmapMonthData.reduce((s, d) => s + d.count, 0)}
                    </span>
                    <span style={{ fontSize: 11, color: 'var(--ink-3)' }}>обновлений</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* ===== TEAM MODE (managers only) ===== */}
      {mode === 'team' && isManager && (
        <TeamAnalyticsTab managerTeams={managerTeams} />
      )}
