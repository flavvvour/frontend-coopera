...
function TaskDetailModal({ task, members, currentUserId, canEditTasks, canDeleteTasks, deleting, onClose, onEdit, onDelete, onProfileClick }: TaskDetailModalProps) {
  ...
  const currentUsername = sessionStorage.getItem('username') ?? 'Участник';
          ...
          <aside className="kb-detail-sidebar">
            ...
            {task.priority && task.priority !== 'low' && (
              <KbSidebarField label="Приоритет">
                <span style={{ fontSize: 12, fontWeight: 600, color: PRIORITY_META[task.priority as Priority]?.color ?? 'var(--ink-2)' }}>
...
export function KanbanBoard({
  ...
  const { deleteTask, loading: deleting } = useHookDeleteTask();
  const { updateTask, loading: updating, error: updateError } = useHookUpdateTask();
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      setLocalTasks(prev => prev.map(t =>
        t.id === selected.id ? { ...t, title: editTitle, description: editDescription, points: editPoints, tags: editTags, priority: editPriority, updatedAt: new Date().toISOString() } : t
      ));
  ...
  const handleDrop = async (colId: ColId, e: React.DragEvent) => {
    e.preventDefault();
    ...
    setLocalTasks(prev => prev.map(t =>
      t.id === taskId ? { ...t, status: colId, updatedAt: new Date().toISOString() } : t
    ));
    ...
    try {
      ...
      setLocalTasks(prev => prev.map(t => t.id === taskId ? { ...t, status: from } : t));
    }
  };
    ...
    <div className="kb-wrapper">
      ...
      {showDetail && selected && (
        <TaskDetailModal
          ...
          onProfileClick={id => setProfileMemberId(id)}
        />
      )}
      ...
      {profileMemberId !== null && (
        <ProfileModal
          ...
          userId={members.find(m => m.id === profileMemberId)?.userId}
          memberUsername={getMemberName(profileMemberId)}
          teamName={teamName}
          memberRole={members.find(m => m.id === profileMemberId)?.role}
          tasks={localTasks}
